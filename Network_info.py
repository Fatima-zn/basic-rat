...
class NetworkInfo:
    ...
    def get_linux_active_connections(self):
        connections = []
        try:
            # Configuration pour masquer la fenêtre CMD sous Windows
            ...
            result = subprocess.run(['netstat', '-tun'], **kwargs)
            for line in result.stdout.split('\n'):
    ...
    def get_linux_listening_ports(self):
        ...
        try:
            ...
            result = subprocess.run(['netstat', '-tunl'], **kwargs)
            for line in result.stdout.split('\n'):
    ...
    def get_windows_network_config(self):
        ...
        try:
            ...
            try:
                # Configuration pour masquer la fenêtre CMD sous Windows
                kwargs = {'capture_output': True, 'text': True, 'shell': True, 'timeout': 10}
                if platform.system().lower() == 'windows':
                    startupinfo = subprocess.STARTUPINFO()
                    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                    startupinfo.wShowWindow = subprocess.SW_HIDE
            ...
            try:
                ...
                result = subprocess.run(['ipconfig', '/all'], **kwargs)
                dns_servers = []
                for line in result.stdout.split('\n'):
    ...
    def get_windows_active_connections(self):
        ...
        try:
            ...
            result = subprocess.run(['netstat', '-n'], **kwargs)
            for line in result.stdout.split('\n'):
                if "ESTABLISHED" in line and ("TCP" in line or "UDP" in line):
                    parts = line.split()
                    if len(parts) >= 4:
    ...
    def get_windows_sensitive_data(self):
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(['netsh', 'wlan', 'show', 'profiles'], **kwargs)
    ...
    def get_mac_addresses(self):
        ...
        try:
            if system == 'windows':
                # Configuration pour masquer la fenêttre CMD sous Windows
                kwargs = {'capture_output': True, 'text': True, 'shell': True, 'timeout': 10}
                if platform.system().lower() == 'windows':
                    startupinfo = subprocess.STARTUPINFO()
                    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
